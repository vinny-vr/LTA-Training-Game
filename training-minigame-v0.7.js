...
function resolveTraining(code, forced) {
  ...
  if (code === "R") {
    ...
    if (injured) {
      if (success) {
        ...
        injuryGain = 0;
        game.stats = Math.ceil(startStats * 0.95);
        game.failures++;
      }
  ...
  if (code === "R") {
    ...
    summary += `Training failed and ${game.name} was injured, so no stats were gained before losing <span class="bad">5% of the current total</span>.`;
  } else if (success) {
